...
# start: random degradation info (DASR)
def random_bivariate_Gaussian_info(
    kernel_size,
    sigma_x_range,
    sigma_y_range,
    rotation_range,
...
def random_bivariate_generalized_Gaussian_info(
    kernel_size,
    sigma_x_range,
    sigma_y_range,
    rotation_range,
...
def random_bivariate_plateau_info(
    kernel_size,
    sigma_x_range,
    sigma_y_range,
    rotation_range,
...
# end: random degradation info (DASR)
...
def random_add_gaussian_noise_pt_info(
    img, sigma_range=(0, 1.0), gray_prob=0, clip=True, rounds=False
):
    # noise = random_generate_gaussian_noise_pt(img, sigma_range, gray_prob)
    sigma, gray_noise, noise = random_generate_gaussian_noise_pt_info(
        img, sigma_range, gray_prob
    )  # DASR
    out = img + noise
    if clip and rounds:
        out = torch.clamp((out * 255.0).round(), 0, 255) / 255.0
...
def only_generate_gaussian_noise_pt_info(img, sigma_range=(0, 1.0), gray_prob=0):
    _, _, noise = random_generate_gaussian_noise_pt_info(
        img, sigma_range, gray_prob
    )  # DASR
    return noise
...
def random_add_poisson_noise_pt_info(
    img, scale_range=(0, 1.0), gray_prob=0, clip=True, rounds=False
):
    # noise = random_generate_poisson_noise_pt(img, scale_range, gray_prob)
    scale, gray_noise, noise = random_generate_poisson_noise_pt_info(
        img, scale_range, gray_prob
    )  # DASR
    out = img + noise
    if clip and rounds:
        out = torch.clamp((out * 255.0).round(), 0, 255) / 255.0
...
def only_generate_poisson_noise_pt_info(img, scale_range=(0, 1.0), gray_prob=0):
    _, _, noise = random_generate_poisson_noise_pt_info(
        img, scale_range, gray_prob
    )  # DASR
    return noise
