...
from starsrgan.utils.file import FileClient
from starsrgan.utils.transform import augment
from starsrgan.utils.logger import get_root_logger
from starsrgan.utils.registry import DATASET_REGISTRY
from starsrgan.utils.img import imfrombytes, img2tensor

from starsrgan.utils.degradation import (
    circular_lowpass_kernel,
    random_mixed_kernels_info,
)


@DATASET_REGISTRY.register()
class StarSRGANDataset(data.Dataset):
    """Dataset used for StarSRGAN model.
    ...
    def __init__(self, opt):
        super(StarSRGANDataset, self).__init__()
        self.opt = opt
        ...
        self.blur_kernel_size2_minimum = opt["blur_kernel_size2_minimum"]  # DASR
        ...
        # # start: normal degradation settings
        # self.kernel_range = [2 * v + 1 for v in range(3, 11)]  # kernel size ranges from 7 to 21
        # # TODO: kernel range is now hard-coded, should be in the configure file
        # self.pulse_tensor = torch.zeros(21, 21).float()  # convolving with pulse tensor brings no blurry effect
        # self.pulse_tensor[10, 10] = 1
        # # end: normal degradation settings

        # start: adaptive degradation settings (DASR)
        self.kernel_range = [
            2 * v + 1
            for v in range(
        ...
        self.blur_kernel_size_minimum_standard1 = opt[
            "blur_kernel_size_minimum_standard1"
        ]
        ...
        # end: adaptive degradation settings (DASR)
    ...
    def __getitem__(self, index):
        ...
        # start: additional adaptive degradation code (DASR)
        ...
        if np.random.uniform() < self.opt["sinc_prob"]:
            ...
            kernel_info = random_mixed_kernels_info(  # DASR
                self.kernel_list,
                self.kernel_prob,
                kernel_size,
                self.blur_sigma,
            ...
            kernel = kernel_info["kernel"]  # DASR
        ...
        # kernel_size = random.choice(self.kernel_range)
        kernel_size = random.choice(self.kernel_range2)  # DASR
        ...
        if np.random.uniform() < self.opt["sinc_prob2"]:
            ...
            # kernel2 = random_mixed_kernels(
            kernel2_info = random_mixed_kernels_info(  # DASR
                self.kernel_list2,
                self.kernel_prob2,
                kernel_size,
                self.blur_sigma2,
        ...
        return_d["severe_degrade_two_stage"] = {
            "gt": img_gt,
            "kernel1": kernel_info,
            "kernel2": kernel2_info,
        ...
        return_d["standard_degrade_one_stage"] = {
            "gt": img_gt,
            "kernel1": kernel_info,
            "gt_path": gt_path,
        }
        ...
        return_d["weak_degrade_one_stage"] = {
            "gt": img_gt,
            "kernel1": kernel_info,
            "gt_path": gt_path,
        }
